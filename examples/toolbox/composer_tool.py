...
class ComposerTool(BaseTool):
    ...
    def __init__(self, tool_id: str = None, config: Dict[str, Any] = None):
        ...
        # Load LLM input composition config from central config (overridable by tool config)
        llm_input_cfg = get_config().get_llm_input_config()
        self.max_input_tokens = self.config.get("max_input_tokens", llm_input_cfg.get("max_input_tokens", 8000))
        self.token_char_ratio = self.config.get("token_char_ratio", llm_input_cfg.get("token_char_ratio", 4))
        self.enable_truncation = self.config.get("enable_truncation", llm_input_cfg.get("enable_truncation", True))
        self.max_sources = self.config.get("max_sources", llm_input_cfg.get("max_sources", 0))
        
        print("------------------------------------------------------------------------------------------------")
        print(f"max_input_tokens: {self.max_input_tokens}")
        print(f"token_char_ratio: {self.token_char_ratio}")
        print(f"enable_truncation: {self.enable_truncation}")
        print(f"max_sources: {self.max_sources}")
        print("------------------------------------------------------------------------------------------------")
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # Check token limits with central-configurable parameters
            if self.enable_truncation:
                max_chars = int(self.max_input_tokens * self.token_char_ratio)
    ...
    def _prepare_content(self, content: Union[str, List[str], List[Dict]], task: str) -> str:
        ...
        if isinstance(content, str):
            ...
            if isinstance(content[0], str):
                separator = "\n\n" + ("=" * 80) + "\n\n"
                items = content
                if self.max_sources and self.max_sources > 0:
            ...
            # Handle list of dictionaries (e.g., web page results)
                ...
                separator = "\n\n" + ("=" * 80) + "\n\n"
                return separator.join(formatted_content)
    ...
    async def _call_llm(self, prompt: str, max_tokens: int, task: str, query: str) -> str:
        ...
        try:
            ...
            # Prepare parameters including all relevant input context
            llm_parameters = {
                "prompt": prompt,
                "max_tokens": max_tokens,
                "temperature": self.temperature,
                "operation_type": operation_type,
                # Provide fields for richer persistence/observability
                "system_prompt": "",
                "messages": []
            }

            # Execute LLM with tracking so inputs/outputs are persisted by BaseTool
            result = await self.llm_tool.execute_with_tracking(
                llm_parameters,
                context={
    ...
    def get_supported_operations(self) -> List[str]:
        ...
        return ["compose", "summarize", "analyze", "combine", "text_generation"]
    ...
    def get_tags(self) -> List[str]:
        ...
        return ["composer", "llm", "text_generation", "summarization", "analysis"]
    ...
    def _get_optional_parameters(self) -> List[str]:
        ...
        return ["query", "format", "max_length", "include_sources"]
    ...
    def _display_result_body(self, result: Any, context: Optional[Dict[str, Any]] = None) -> None:
        print("✍️ CONTENT COMPOSITION RESULTS:")
